	...
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' v-if="type!='info'"  label="举报理由" prop="jubaoliyou">
					<editor 
						style="min-width: 200px; max-width: 600px;"
						v-model="ruleForm.jubaoliyou" 
						class="editor" 
						action="file/upload">
					</editor>
				</el-form-item>
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' v-else-if="ruleForm.jubaoliyou" label="举报理由" prop="jubaoliyou">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.jubaoliyou"></span>
                </el-form-item>
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='jubaoliyou'){
							this.ruleForm.jubaoliyou = obj[o];
							this.ro.jubaoliyou = true;
							continue;
						}
						if(o=='yonghuzhanghao'){
	...
	this.ruleForm.jubaoliyou = this.ruleForm.jubaoliyou.replace(reg,'../../../springboot1490x/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `wenzhangjubao/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.wenzhangjubaoCrossAddOrUpdateFlag = false;
						 this.parent.search();
